...
void sendLog(LogLevel level, const char* msg) {
  ...
  if (msgLen > 240) msgLen = 240; // Safe buffer limit
...
void sendTelemetryPacket() {
    ...
    pkt.speed = speed; // Assuming speed variable is current speed
...
void loop() {
  ...
  if (!errorStatus[0] && status == 25) {
    ...
    if (millis() - count > 1000 && !motorStart) {
      ...
      IWatchdog.reload();
      if (reportCounter == 10) {
        // sensor_Report(); // Removed, handled by periodic telemetry
        reportCounter = 0;
      }
      send_Cmd();
      
      // makeLog(LOG_INFO, "online..."); // Optional, maybe redundant with heartbeat
      // delay(20);
      // Serial1.println("CB " + String(batteryCharge));
      // delay(20);
      // Serial1.println("CV " + String(batteryVoltage));
        ...
        switch (state) {
            ...
            case STATE_WAIT_SYNC2:
                if (byte == PROTOCOL_SYNC_2) {
                    rxBuffer[1] = byte;
                    rxIndex = 2; // Next byte is at index 2
...
void set_Position() {
  ...
  if (motorReverse == 0 ^ inverse) {
    if (oldAngle - angle > 0 && oldAngle - angle <= 2000) {
      ...
      diff = (angle * calibrateEncoder_F[0] + (4096 - oldAngle) * calibrateEncoder_F[7]) / 512; /*Serial1.print("[3] dAng>2000 ");*/
    }
    if (diff > 0 && !inverse) {
    ...
    if (millis() - count > 50) {
      ...
      if (inverse) {
        if (startAngle - currentAngle > 0 && startAngle - currentAngle <= 2000) {
          ...
          diff = (startAngle * (int)calibrateEncoder_F[0] + (4096 - currentAngle) * (int)calibrateEncoder_F[7]) / 512; /*Serial1.print("[9] ");*/
        } else if (currentAngle - startAngle > 0 && currentAngle - startAngle <= 2000) {
          uint8_t s = startAngle / 512;
          uint8_t f = currentAngle / 512;
