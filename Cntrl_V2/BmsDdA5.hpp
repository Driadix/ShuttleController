...
namespace BmsDdA5
{
    ...
    struct Hooks
    {
        ...
        Hooks() : write(0), readByte(0), setDriverEnable(0), user(0) { }
    };
    ...
    {
      public:
        explicit Client(const Hooks &hooks, const Config &config = Config()) : hooks_(hooks), config_(sanitizeConfig(config))
        {
        ...
        {
            if (snapshot_.lastBasicUpdateMs == 0U)
                return false;
            if (config_.staleWarnMs == 0U)
                return true;
      ...
      private:
        ...
        bool checksumOk(uint8_t payloadLen) const
        {
            ...
            uint16_t calc2 = 0U;
            for (uint8_t i = 2U; i < static_cast<uint8_t>(4U + payloadLen); i++)
                calc2 += rxFrame_[i];
            calc2 = static_cast<uint16_t>(~calc2 + 1U);

            uint16_t calc3 = 0U;
            for (uint8_t i = 3U; i < static_cast<uint8_t>(4U + payloadLen); i++)
                calc3 += rxFrame_[i];
        ...
        bool parseBasicInfo(uint8_t payloadLen)
        {
            ...
            for (uint8_t i = 0U; i < kStoredNtcCapacity; i++)
                snapshot_.ntcTemp_dC[i] = 0;

            uint8_t ntcToStore = snapshot_.ntcCount;
            if (ntcToStore > config_.maxStoredNtc)
                ntcToStore = config_.maxStoredNtc;
            if (ntcToStore > kStoredNtcCapacity)
                ntcToStore = kStoredNtcCapacity;

            for (uint8_t i = 0U; i < ntcToStore; i++)
            {
                const uint8_t idx = static_cast<uint8_t>(23U + i * 2U);
                if (static_cast<uint8_t>(idx + 1U) >= payloadLen)
                    break;
        ...
        {
            if ((payloadLen & 0x01U) != 0U)
                return false;

            const uint8_t *p     = &rxFrame_[4];
            uint8_t        cells = static_cast<uint8_t>(payloadLen / 2U);
            if (cells > config_.maxStoredCells)
                cells = config_.maxStoredCells;
            if (cells > kStoredCellCapacity)
                cells = kStoredCellCapacity;
            ...
            for (uint8_t i = cells; i < kStoredCellCapacity; i++)
                snapshot_.cellMv[i] = 0U;
        ...
        bool processFrame(uint32_t nowMs, uint32_t *events)
        {
            ...
            if (cmd == static_cast<uint8_t>(RequestKind::BasicInfo))
            {
                ...
                if (parsed)
                {
                    ...
                    if (events)
                        *events |= eventMask(Event::BasicUpdated);
                }
            }
            else if (cmd == static_cast<uint8_t>(RequestKind::CellInfo))
            {
                ...
                if (parsed)
                {
                    ...
                    if (events)
                        *events |= eventMask(Event::CellUpdated);
                }
            }
            else if (cmd == static_cast<uint8_t>(RequestKind::DeviceInfo))
            {
                ...
                if (parsed)
                {
                    ...
                    if (events)
                        *events |= eventMask(Event::DeviceUpdated);
                }
            }
            else
            {
