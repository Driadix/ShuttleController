...
namespace BmsDdA5Firmware
{
    ...
    class Adapter
    {
      ...
      private:
        ...
        {
            if (lastMs == kNeverMs || intervalMs == 0U || static_cast<uint32_t>(nowMs - lastMs) >= intervalMs)
            {
                lastMs = nowMs;
                return true;
            }
        ...
        void logWarnEvents(uint32_t nowMs, uint32_t events)
        {
            ...
            {
                if (snapshot.hasCell && snapshot.seriesCellCount > 0U && snapshot.cellCount > 0U &&
                    snapshot.seriesCellCount != snapshot.cellCount &&
                    due(nowMs, warnState_.lastCountMismatchMs, warnConfig_.countMismatchMs))
                {
                    logFn_(LOG_WARN, "BAT cnt 03=%u 04=%u", snapshot.seriesCellCount, snapshot.cellCount);
                }

                if (snapshot.protectionFlags != 0U && due(nowMs, warnState_.lastProtectionMs, warnConfig_.protectionMs))
                {
                    logFn_(
                        LOG_WARN,
                        "BAT prot=%04X fet=%02X soc=%u",
            ...
            if (BmsDdA5::hasEvent(events, BmsDdA5::Event::CellUpdated))
            {
                ...
                {
                    const int32_t diffMv =
                        static_cast<int32_t>(snapshot.cellSum_mV) - static_cast<int32_t>(snapshot.packVoltage_mV);
                ...
                if (snapshot.cellMin_mV > 0U && snapshot.cellMin_mV < warnConfig_.cellLowWarn_mV &&
        ...
        void logStaleWarning(uint32_t nowMs)
        {
            ...
            if (snapshot.lastValidFrameMs == 0U)
            {
                if (nowMs >= cfg.staleWarnMs &&
                    (!warnState_.staleWarned || static_cast<uint32_t>(nowMs - warnState_.lastStaleMs) >= cfg.staleRepeatMs))
                {
