...
namespace E32Radio
{
    ...
    inline RawConfig encode(const LogicalConfig &logical)
    {
        ...
        raw.sped = static_cast<uint8_t>(
            ((static_cast<uint8_t>(logical.parity) & 0x03u) << 6) | ((static_cast<uint8_t>(logical.uartBaud) & 0x07u) << 3) |
        ...
        raw.option = static_cast<uint8_t>(
            ...
            ((static_cast<uint8_t>(logical.wakeupTime) & 0x07u) << 3) | ((static_cast<uint8_t>(logical.fec) & 0x01u) << 2) |
    ...
    {
        LogicalConfig cfg =
            makeTransparentConfig(address, channel, uartBaud, airDataRate, txPower, parity, ioDrive, wakeupTime, fec);
    ...
    class Radio
    {
      public:
        Radio()
            : _serial(nullptr), _modePin(0), _runtimeBaud(9600), _configModeLevelHigh(false), _initialized(false),
              _logger(nullptr)
        {
        }
      ...
      private:
        ...
        {
            return lhs.addh == rhs.addh && lhs.addl == rhs.addl && lhs.sped == rhs.sped && lhs.chan == rhs.chan &&
